...
#define TMPpcmReadyLow() TMPpcm_READY_PORT &= ~_BV(TMPpcm_READY_BIT) //*vad
#define TMPpcmReadyHigh() TMPpcm_READY_PORT |= _BV(TMPpcm_READY_BIT) 
...
void TMRpcm::setPin(){
	...
	TMPpcm_READY_DDR |= _BV(TMPpcm_READY_BIT);     // set sport to output mode   //*vad 
...
void TMRpcm::pause(){
	//paused = !paused;
	if(bitRead(optionByte,7) && playing){ // start
		...
		#endif
	}else if(!bitRead(optionByte,7) && playing){ // stop
		bitSet(optionByte,7);
		//*TIMSK[tt] &= ~( _BV(TOIE1) );
		*TCCRnA[tt] &= ~(_BV(WGM11) | _BV(COM1A1)); //WGM11,12,13 all set to 0 -> no compare
		*TIMSK[tt] &= ~(_BV(ICIE1) | _BV(TOIE1));
	}
}
...
boolean TMRpcm::isPause(){
	return bitRead(optionByte,7); //***vad
}
